...
The latest version, PHILOCHESS 0.2-alpha-011, introduces recognition of the fifty-move rule throughout the search. The engine now uses the halfmove clock stored in the FEN and updates it after every move, incrementing it for non-pawn, non-capturing moves and resetting it for pawn moves or captures. When the counter reaches or exceeds 100 plies, PHILOCHESS treats the position as a draw for search purposes at the root, in alpha-beta search, and in quiescence search. Checkmate, stalemate, and dead positions retain their precedence and diagnostic identity. At a draw-eligible root with legal moves, the engine still returns a legal bestmove in its UCI output while bypassing unnecessary static evaluation and further search. The implementation was validated against a dedicated test inventory and the established regression suite. Development will continue with PHILOCHESS 0.2-alpha-012: Threefold Repetition Detection in Search.
