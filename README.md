...
The latest version, PHILOCHESS 0.2-alpha-012, introduces detection of threefold repetition throughout the search. The engine now maintains a chronological position history reconstructed from UCI position commands and extends that history independently along each search path. Repetition identity is determined by piece placement, side to move, castling rights, and effective en passant availability; the FEN halfmove and fullmove counters do not affect identity, and an en passant square is included in the repetition identity only when a legal en passant capture is available. When the same position occurs for the third time or later, PHILOCHESS treats it as a draw for search purposes at the root, in alpha-beta search, and in quiescence search, returning a score of zero while preserving branch isolation and the established handling of legal moves and terminal positions. Development will continue with PHILOCHESS 0.2-alpha-013: Draw-Rule Integration and Cross-Rule Regression.
