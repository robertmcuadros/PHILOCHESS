...
The latest version, PHILOCHESS 0.3-alpha-002, completes the Engine Modularization and Interface Separation milestone. The former monolithic codebase has been reorganized into eight functional modules covering core infrastructure, chess logic, evaluation, rules, search, UCI communication, diagnostics, and testing, with explicit headers and interfaces defining the boundaries between components. The new architecture preserves the full functional behavior inherited from alpha-001, including search decisions, evaluation, time controls, draw-rule handling, UCI contracts, asynchronous search control, and Release/Debug semantic parity. Alpha-002 is an architectural transformation rather than a playing-strength improvement, establishing a maintainable foundation for the rest of the PHILOCHESS 0.3 development cycle. Development will continue with PHILOCHESS 0.3-alpha-003: Reproducible Benchmarking, Profiling, and Statistical Match Analysis.
